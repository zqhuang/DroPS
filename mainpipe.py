...
if(ana.BB_loc >= 0):
    params['r'] = [ r'$r$', 0., 0.2, 0.015, 0.004 ]
else:
    params['r'] = [ r'$r$', 0., 0. ]
...
if(ana.ME_is_positive):
    ...
    ME_lbd = - ME_ubd
ME_ini = (ME_ubd+ME_lbd)/2.
ME_step = (ME_ubd-ME_lbd)/20.


if(ana.dust_vary_SED):
    for field in ana.fields:
        if(ana.analytic_ME):
            params[r'B_d_'+field] = [r'$B_{d,' + field + r'}$', ME_lbd, ME_ubd, ME_ini, ME_step]            
            params[r'S_d_'+field] = [ r'$S_{d' + field + r'}$', ME_lbd, ME_ubd, ME_ini, ME_step]
            params[r'gamma_d_'+field] = [ r'$\gamma_{d,' + field + r'}$', -5., 0., -0.5, 0.1 ]
        else:
            for i in range(ana.num_ells):
                params[r'B_d_'+field+str(i)] = [r'$B_{d,'+field+r','+str(i)+r'}$', ME_lbd, ME_ubd, ME_ini, ME_step]                
                params[r'S_d_'+field+str(i)] = [r'$S_{d,'+field+r','+str(i)+r'}$', ME_lbd, ME_ubd, ME_ini, ME_step]
else:
    for field in ana.fields:
        if(ana.analytic_ME):
...
if(ana.sync_vary_SED):
    for field in ana.fields:
        if(ana.analytic_ME):
            params[r'B_s_'+field] = [r'$B_{s,' + field + r'}$', ME_lbd, ME_ubd, ME_ini, ME_step]            
            params[r'S_s_'+field] = [ r'$S_{s' + field + r'}$', ME_lbd, ME_ubd, ME_ini, ME_step]
            params[r'gamma_s_'+field] = [ r'$\gamma_{s,' + field + r'}$', -5., 0., -0.5, 0.1]
        else:
            for i in range(ana.num_ells):
                params[r'B_s_'+field+str(i)] = [r'$B_{s,'+field+r','+str(i)+r'}$', ME_lbd, ME_ubd, ME_ini, ME_step]                                
                params[r'S_s_'+field+str(i)] = [r'$S_{s,'+field+r','+str(i)+r'}$', ME_lbd, ME_ubd, ME_ini, ME_step]
else:
    for field in ana.fields:
        if(ana.analytic_ME):
...
if(ana.analytic_fg):
    ...
    for field in ana.fields:
        ...
        eps_approx = ana.select_spectrum(vec = ana.data_vec- ana.lens_res, ifreq1 = ana.ifreq_lowest, ifreq2 = ana.ifreq_highest, field = field) / ana.beam_filter2(ifreq1 = ana.ifreq_lowest, ifreq2 = ana.ifreq_highest) / np.sqrt(sync_approx * dust_approx) / (fgm.sync_freq_weight(ana.freq_lowest) * fgm.dust_freq_weight(ana.freq_highest)+fgm.sync_freq_weight(ana.freq_highest) * fgm.dust_freq_weight(ana.freq_lowest))
        for i in range(ana.num_ells):
            if(not( abs(eps_approx[i]) < ana.eps_upperbound and sync_approx[i] > 0. and dust_approx[i] > 0.)):
                print(' field = ', field, ' l = ', ana.ells[i], 'A_s = ', sync_approx[i], 'A_d = ', dust_approx[i], 'eps = ', eps_approx[i])                
                print("Warning: failed to use map to estimate the fg params, use some small numbers to approximate!")
                dust_approx[i] = max(0.05, dust_approx[i])                
                sync_approx[i] = max(0.01, sync_approx[i])
                eps_approx[i] = 0.
            if(ana.ell_used_indices[i]):
                params[r'A_s_' + field  + str(i)] = [ r'$A_{s, ' + field + r',' + str(i) + r'}$',  0., sync_approx[i]*4.+5., sync_approx[i], sync_approx[i]/20. ]
                params[r'A_d_' + field  + str(i)] = [ r'$A_{d, ' + field + r',' + str(i) + r'}$',  0., dust_approx[i]*4.+20.,  dust_approx[i], dust_approx[i]/20.]
                params[r'eps2_' + field  + str(i)] = [ r'$\varepsilon_{2, ' + field + r',' + str(i) + r'}$',  -ana.eps_upperbound, ana.eps_upperbound, eps_approx[i], 0.005 ] 
            else:
                ...
                params[r'eps2_' + field  + str(i)] = [ r'$\varepsilon_{2, ' + field + r',' + str(i) + r'}$',  eps_approx[i], eps_approx[i] ]

if(len(argv)>3):
    settings = mcmc_settings(nwalkers = 1, burn_steps = 1500, mc_steps = mc_steps, verbose = True, mcmc_engine = 'simple', covmat=argv[3])        
else:
    settings = mcmc_settings(nwalkers = 1, burn_steps = 1500, mc_steps = mc_steps, verbose = True, mcmc_engine = 'simple')    
...
np.save(bestfit_file, settings.bestfit)
np.save(covmat_file, settings.covmat)

print("-----------mean +/- standard deviation---------")    
for i in range(settings.num_params):
    print(settings.names[i], r'=', settings.mean[i], r'\pm ', settings.std[i], r', best: ', settings.bestfit[i])

print('current min chi^2 = : ', -2.*settings.bestlike) 
print('refining bestfit searching steps...')    
settings.search_bestfit(cmb_loglike)
for i in range(settings.num_params):
    print(r'best ', settings.names[i], r' = ', settings.bestfit[i])
print('now best like =  ', settings.bestlike)                         
print('   min chi^2 = ', -2.*settings.bestlike)
    

with open(ana.output_root + r'margestat.txt', 'w') as f:
    f.write("# name     mean  std  median 95-upper 95-lower bestfit\n")    
    for i in range(settings.num_params):
        median = settings.prob_limits(i, 0.5)
        upper95 = settings.prob_limits(i, 0.95)
        lower95 = settings.prob_limits(i, 0.05)                
        f.write(settings.names[i] + ' ' + str(settings.mean[i]) + ' ' + str(settings.std[i]) + ' ' + str(median) + ' ' + str(upper95) + ' ' + str(lower95) + ' ' + str(settings.bestfit[i]) + "\n")
    f.write("#chi^2/d.o.f.:"+str(-2*settings.bestlike/ana.fullsize))            
    

if(debug):
    fgs = params_to_fg_models(settings.bestfit, settings)
