...
def plot_distr(filename, label, color,  cv):
    ...
    plt.axvline(cv[1], color=color[2], ls = "dashdot", lw = 2.5, label=label+r", Wolz et al. Pipeline B",alpha=0.8)
    plt.axvline(cv[2], color=color[2], ls = "dashed", lw = 2.5, label=label+r", Wolz et al. Pipeline C",alpha=0.8)    
