...
A real experiment comes with its own map making and noise simulation pipelines, and provides reference simulations and the actually observed sky maps. To analyze the real data, you can simply replace the reference simulations with the maps from the simulation pipeline of the experiment, and replace the realization of sky with the actually observed frequency maps.
...
\end{equation}
Because the dependencies of $\tilde{n}_{\ell m}$ on $c_{\ell m}$, $d_{\ell m}$ and $s_{\ell m}$ are all linear, a gradient template can be pre-computed for given mask and map-making pipeline. The task of computing the gradient template, which involves map-making from time-ordered data, is computationally demanding. However, since the processing for each individual $\ell, m$ mode is independent, this task is highly parallelizable and can be efficiently executed on a computing cluster.
