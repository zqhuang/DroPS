...
\author{Zhiqi Huang \\
  ...
  {\small supported by: National key R\&D Program of China (2020YFC2201602)}
}
\date{November 2025} 
\maketitle 
...
and
\begin{equation}
\mathcal{P}_T(k) = r A_s \left(\frac{k}{k_{\mathrm{pivot}}}\right)^{n_t}.
\end{equation}
...
Designed for efficiency, DroPS has minimal hardware requirements. The analyses presented in this documentation were performed efficiently on a conventional laptop. A typical setup for running DroPS is a standard personal computer with a few hundred gigabytes of free storage. Note that, however, for real data analysis which involves running specific map making pipelines, the 
...
Hereafter we assume the user works within a bash terminal.
...
Create a directory for your Python virtual environment within your work path (referred to as YourWorkPath):
\tbox{mkdir YourWorkPath/.work}
The name .work is just an example—you can choose any directory name. However, it is a good practice to start the name with a dot (.), which makes the directory hidden by default. This helps prevent accidentally deleting the Python packages installed in the virtual environment.
...
We are going to hack pysm3, which is one of the packages we just installed with pip. Its main function is to simulate the sky with given foreground and cosmology models.
...
Before analyzing the observed sky maps (whether simulated or real), it is necessary to generate a reference dataset of simulated CMB, noise, and foreground signals. These baseline simulations represent our current best-informed model of reality (incorporating instrument characteristics, noise statistics, foreground properties, etc.). Conceptually, this reference dataset can be likened to the ``training samples'' used in machine learning\footnote{Note that this is purely an analogy; DroPS does not employ advanced machine learning techniques like deep learning.}. However, it is important to note that systematic offsets may still exist between these baseline simulations and the actual observations. Minimizing the potential impact of such discrepancies is a key optimization objective in the design of DroPS.

\subsubsection{Generating a TOD filtering model}
...
\subsubsection{Generating reference simulations}

Now we are ready to generate the reference simulations for given experiments.  DroPS comes with four ``demo-experiments'': Test, AliCPT, SO, and CMBS4.  The ``Test'' demo-experiment is just for testing purpose and does not have a counterpart in reality. The configurations (sky coverage, noise level etc., see Table~\ref{tab:config}) of the latter three demo-experiments (AliCPT, SO, CMBS4) are similar, but not identical to their counterparts in reality (AliCPT~\cite{AliCPT}, SO-SAT~\cite{SO-SAT} and CMB-S4-SAT~\cite{CMB-S4-SAT}). In this documentation we will mainly use the ``AliCPT'' experiment as examples. We will also use SO for comparison between different foreground modeling and between DroPS and other pipelines in the literature~\cite{SO-SAT}. However, it is important to note that at the time of writing this documentation, both the AliCPT and SO experiments have commenced observations. However, their actual experimental parameters and observation strategies differ from those of the demo-experiments bearing the same names in DroPS. Therefore, the test results from DroPS demo-experiments should not be regarded as accurate predictions for the real experiments.
...
to generate reference simulations for the demo-experiment AliCPT.

Compare the content of configuration file AliCPT/AliCPT\_sim\_config.txt with Table~\ref{tab:config} to understand how the configurations are passed to the script. When no other command-line arguments are passed to simulate.py, it only produces reference simulations, that are, a lot of realizations of noise and CMB maps based on the noise model and cosmology that are specified in the configuration file. By default, DroPS employs two distinct values of $r$ ($r=0$ and $r=r_1=0.03$, where the value of $r_1$ can be modified in the configuration file) to generate two separate sets of CMB sky maps. This approach is adopted in order to mitigate potential systematic biases arising from inaccuracies in modeling the time-ordered data filtering. For a detailed explanation, please refer to Section \ref{sec:cmb}.

The foreground maps are only produced once, as we do not yet have a reliable model to describe the ``cosmic variance'' of foreground emissions. This reflects the most critical technical bottleneck in current efforts to measure primordial gravitational waves: we currently possess neither a theoretical model derived from first principles nor sufficient multi-frequency measurement data to establish a reliable statistical model for describing foreground spectral fluctuations.

By default, the reference simulations use the foreground model ['d0', 's0'] with fixed spectral indices ($\beta_d=1.54$ for thermal dust, $\beta_s=-3$ for synchrotron). This ['d0', 's0'] foreground map only captures the gross feature of the Galactic emission. The ``actual foreground'' that we will analyze in the next section can be different from the one used in the reference simulations. A robust $r$ measurement pipeline must be insensitive to the finer details of the foreground modeling.
...
In the previous section, we generated a set of reference simulations. These serve as a reference library, constructed from a specific noise model, the foreground model ['d0', 's0'], and a set of fiducial $r$ values within a $\Lambda$CDM cosmology. Now we can simulate an ``observed sky'' map. This mock observation uses the same noise model but can employ a different foreground model and, crucially, an $r$ value that is independent of those in the reference simulations. DroPS will determine the value of $r$ in this ``observed sky'' by statistically comparing it against the reference simulation library. Conceptually, this ``observed sky'' is analogous to a ``test sample'' in machine learning. We may also generate many realizations of the ``observed sky'' to assess the average offset (bias) between the reconstructed and input $r$ values.

\subsubsection{Analyzing one realization of sky}

Let us first generate an ``observed sky map'':
\tbox{python simulate.py AliCPT/AliCPT\_sim\_config.txt maps/AliCPT\_ 0.01 999}
We are now passing multiple parameters to simulate.py from the command line. The first parameter remains the configuration file. The second parameter maps/AliCPT\_ serves as the filename prefix for storing the observed sky maps (for example, the 40GHz sky map will be saved as AliCPT/maps/AliCPT\_40GHz.npy). The third parameter 0.01 is the user-specified value of $r$. The fourth parameter 999 is the random seed.

If users wish to employ a foreground model different from the ['d0', 's0'] specified in the configuration file, there are two approaches: one is to directly modify the configuration file by replacing ['d0', 's0'] with, for instance, ['d1', 's1']. Alternatively, a fifth parameter can be appended to the command line, such as 'd1s1'.
...
\begin{figure}
  ...
  \caption{Reconstructed $r$ for 100 realizations of sky with different random seeds. For each sky, the $r$ value is reconstructed by comparing the sky with 300 reference simulations. The 100 realizations of sky are simulated with foreground model ['d1','s1'] (spatially varying SED of synchrotron and dust emission), while  ['d0', 's0'] (fixed SED) is used in the 300 reference simulations. \label{fig:r_logs}}
\end{figure}


\subsubsection{Applying DroPS on real data}

In reality, each experiment has its own map-making and noise simulation pipeline, which can provide reference simulations for noise and CMB. Additionally, the experiment will supply the observed sky maps. As long as these files are named according to DroPS' naming conventions, they can be analyzed using DroPS. 

By default, DroPS includes two sets of CMB reference simulations: one with $r=0$ and another with $r=r_1=0.03$. If an experiment provides only the CMB reference dataset for $r=0$, set cosmo\_r1 to None in the analysis configuration file. However, if computational resources allow, the ideal approach is to generate an additional CMB reference simulation for $r=r_1$.

Real experiments have finite bandwidth for each frequency channel, characterized by a frequency distribution function $f(\nu)$. In such cases, you can define the path to a directory, band\_weights\_root, in the configuration file and store the distribution functions for each frequency channel in this directory. For the naming and format of these files, please refer to the Test demo-experiment as a reference.

\subsubsection{Comparison with other pipelines}

DroPS has been applied on AliCPT data challenge  and achieved consistent results with the simulation input~\cite{Zhang24}.

Here we further compare DroPS with three pipelines that have been applied on simulation data of SO-SAT (Wolz et al. 2024~\cite{SO-SAT}). We adopt the optimistic configuration of SO-SAT. The input fiducial $r$ is zero, and in the analyses unphysical negative $r$ is allowed\footnote{in DroPS this is realized by setting r\_lowerbound negative in the configuration file}.  The results are shown in Figure~\ref{fig:compare_SO}.  From top to bottom, foreground models ['d0', 's0'], ['d1', 's1'], and ['dm', 'sm'] are used, respectively\footnote{The model ['dm', 'sm'] does not come with pysm3 and needs to be prepared by the user. See~\cite{SO-SAT} for its definition.}. From the left column, where the reconstructed means of $r$ and the bias (mean of the reconstructed means of $r$) are shown, we find DroPS has negligible biases for all cases and more stable than the three pipelines tested in Wolz et al. The right column shows $\sigma_r$, the statistical uncertainties in $r$. We again find good consistency between DroPS and the other pipelines. 
...
In this section we consider component separation at the map level. Our target is to obtain the CMB E and B maps from the observed multi-frequency sky maps.
...
DroPS also provides a map-level component separation code. To demonstrate how the code works, we generate a sky realization from the reference simulation \#0:
\tbox{{\small python utils/combine\_sim.py AliCPT/AliCPT\_sim\_config.txt maps/r3AliCPT0\_  0}}
The above script takes the reference simulation \#0 and adds the filtered foreground maps, filtered noise maps and filtered CMB maps into sky frequency maps with root name maps/r3AliCPT0\_.

Now we do component separation
\tbox{ python compsep.py AliCPT/AliCPT\_sim\_config.txt maps/r3AliCPT0\_}
\noindent Note that this step takes about 100GB disk space and about 10GB memory.

The code uses Stochastic Gradient Langevin Dynamics (SGLD) method to search the maximum of the likelihood described in Sec.~\ref{sec:sgld_like}. It typically takes a few days to run compsep.py for the first time, on a personal computer. When the gradient templates are built, the code runs much faster, typically takes only a few hours on a personal computer. Typically you need to run it multiple times to obtain a well converged CMB B-mode map.

Figure~\ref{fig:compsep} presents the results of the execution. The reconstruction of the $E$-mode map is remarkably accurate, whereas significant residual noise persists in the $B$-mode map. Additionally, the filtering of the time-ordered data leads to some loss of information on the largest angular scales\footnote{For CMB $E$ map, this effect is relatively less pronounced because the power spectrum of the CMB $E$-modes at the largest angular scales is relatively low.}.
...
Thus, with parallel I/O that is commonly supported in modern architectures, the task can be decomposed into many sub-tasks with different lmin and lmax.



\subsubsection{Visualization of partial-sky maps}

To save disk space, masked maps are saved in npy files, in which only unmasked pixels are saved. These partial sky maps cannot be directly displayed on the screen with the standard Healpix tools. DroPS provides a few tools to display them.
...
Now suppose the mask file is mask.fits. To display the masked-sky maps in npy format, using utils/viewnpy.py:
...
Suppose there are $n_{\rm bin}$ $\ell$-bins, $n_\nu$ frequency channels, and $n_{\rm fld}$​ field pairs (e.g., TT, TE, EE, BB, etc.). The data vector $D^{XY}_\ell(\nu_1,\nu_2)$ used in DroPS is ordered by the following indices:
\begin{itemize}
\item{The central values of the $n_{\rm bin}$ $\ell$-bins;}
\item{The $n_\nu(n_\nu+1)/2$ ordered frequency pairs $(\nu_1, \nu_2)$ (with $\nu_1 \le \nu_2$);}
\item{The $n_{\rm fld}$ field pairs $XY$.}
\end{itemize}
As mentioned earlier, for $\nu_1 = \nu_2$​, the band power  $D^{XY}_\ell(\nu_1,\nu_2)$ is obtained by averaging the cross-correlations between maps from different observation seasons.
    
The full data vector length is then $N_d = \frac{1}{2} n_{\rm fld} n_{\rm bin} n_\nu (n_\nu+1)$. For $r$-measurement with limited computing or storage resources, we recommend using only the $BB$ spectra ($n_{\rm fld}=1$), as they contain nearly all the information on $r$.
 ...
 Prior to discussing the specific noise, CMB, and foreground models or the technical estimation of $\mathrm{Cov}$, we will outline two key components: the map-making simulators for the reference simulations, and how the TOD filtering is modeled throughout the pipeline. 



\subsection{Map making and TOD filtering\label{sec:TOD}}
...
\item{Multiply the input map pixel-wise by the smoothed mask.}
...
It is important to note that the analytic model in Eq.~\eqref{eq:noise_model} is used exclusively for simulations. During the data analysis stage, DroPS bypasses this model and computes the noise covariance directly from the reference simulations themselves. Furthermore, the expectation value of the noise power spectrum is always zero in the analysis because DroPS exclusively employs cross-power spectra between different frequency channels or observation seasons, thereby inherently deriving noise-debiased power spectra.
...
DroPS assumes standard Lambda cold dark matter ($\Lambda$CDM) with an additional parameter $r$. For the demo experiments, the default cosmological parameters for reference simulations are listed in Table~\ref{tab:cosmo}. 

\begin{table}
  \centering
  \caption{Default cosmological parameters for reference simulations \label{tab:cosmo}}
  \begin{tabular}{lll}
    \hline
    \hline
...
\end{table}
 ...
 Unlike the noise statistics, which can be empirically derived from the reference simulations, the CMB statistics depend on underlying cosmological parameters—most notably the unknown tensor-to-scalar ratio $r$. This fundamental difference necessitates a more sophisticated approach for modeling the CMB band powers and their contribution to the signal covariance matrix.
...
\subsubsection{Cosmological parameter inference}
    ...
    The primary goal of DroPS is to infer the tensor-to-scalar ratio $r$, which is by default the only free cosmological parameter. Users can choose to vary the full set of standard $\Lambda$CDM parameters by setting vary\_cosmology = True in the configuration file. However, since the large-scale polarization data from ground-base SATs alone provide weak constraints on the background cosmology, DroPS applies a Gaussian prior based on Planck+BAO results~\cite{Planck18Params} to ensure the parameters remain within physically reasonable ranges\footnote{This prior can be disabled by modifying mainpipe.py, but doing so may lead to poor convergence in the Monte Carlo sampling or numerical instability.}.


The reference simulations are generated with two fiducial values of the tensor-to-scalar ratio: $r=0$ and $r=r_1>0$. By averaging over these simulations, we obtain the expected CMB band powers, $\langle D^{\rm CMB}_\ell(r=0)\rangle $ and $\langle D^{\rm CMB}_\ell(r=r_1)\rangle$. The recommended value for $r_1$​ is $0.03$ (approximately the current 90\% confidence level upper limit), which is user-configurable.    

Under the standard cosmological model, and assuming other parameters $\theta$ are fixed to their fiducial values $\theta_{\rm fid}$​, the CMB band powers exhibit an approximately linear response to $r$. DroPS leverages this by modeling the CMB band powers as a linear interpolation between the two simulated benchmarks, which inherently includes all instrumental effects:
\begin{equation}
...
\end{equation}
Note that CMB band powers in the above equation are all theoretical predictions calculated with CAMB~\cite{CAMB}. This design is highly efficient for the most likely scenario in next-generation experiments, where $|r|<r_1$ and cosmological parameters are well-constrained. In this case, the term inside the brackets is small, and the filtering matrix—whose model might be approximate—is applied only to a residual, thereby minimizing the impact of any inaccuracies in the filtering model on the final result.



\subsection{Foreground model \label{sec:fg}}

\subsubsection{Thermal dust emission and synchrotron radiation: base model}
...
\end{equation}
where  $\beta_s$ is the frequency spectral index, and $\nu_{\rm ref}$ is an arbitrarily chosen reference frequency. Because synchrotron radiation dominates the low-frequency bands, we often choose a low reference frequency, e.g.,  $\nu_{\rm ref,sync} \sim 30\,\mathrm{GHz}$.
...
If the number of available frequency channel is small or the noise is large (target $\sigma_r\gtrsim 0.01$), however, smooth parametrization of foreground power is recommended. The reason is that the filter modeling uncertainty and the ``cosmic variance'' of foreground both become a negligible effect compared to noise . The smooth model of foreground band powers - which is modeled as a quadratic function of $\ell$ - can be turned on in the configuration file by setting analytic\_fg = True.
...
The minimal parametrization then involve two unknown spectra  $C_\ell^{\rm ref}$ and $S_\ell$, from which $\mu$ and $\varepsilon_\ell$ can be derived.

In practical CMB experiments, particularly ground-based ones, we must account for the effects of masks that cover only partial sky areas (with sky fraction fskyfsky​). In this context, the power spectra in equations \eqref{eq:ME}, \eqref{eq:ME_mu}, and \eqref{eq:ME_eps} are pseudo power spectra (power spectra of the product of the sky map and the mask), while DroPS operates on band powers, which are estimates of the full-sky statistically isotropic power spectrum. Denoting the pseudo power spectra as $\tilde{S}_\ell$​ and $\tilde{C}_\ell$​, their relationship with the full-sky power spectra is given by~\cite{Master}:
\begin{eqnarray}
\tilde{S}\ell &=& \frac{2\ell^\prime+1}{4\pi}\sum{\ell^\prime, \ell^{\prime\prime}}(2\ell^{\prime\prime}+1)\begin{pmatrix}\ell & \ell^\prime & \ell^{\prime\prime} \ 0 & 0 & 0 \end{pmatrix}^2 M_{\ell^{\prime\prime}}S_{\ell^\prime}, \\
\tilde{C}\ell^{\rm BB} &=& \sum{\ell^\prime} Q_{\ell \ell^\prime}^{-} C_{\ell^\prime}^{BB} + Q_{\ell \ell^\prime}^{+} C_{\ell^\prime}^{EE}, \\
\tilde{C}\ell^{\rm EE} &=& \sum{\ell^\prime} Q_{\ell \ell^\prime}^{+} C_{\ell^\prime}^{EE} + Q_{\ell \ell^\prime}^{-} C_{\ell^\prime}^{BB}.
\end{eqnarray}
Here
\begin{equation}
Q^{\pm}{\ell\ell^\prime} = \frac{2\ell^\prime+1}{4\pi}\sum{\ell^{\prime\prime}}(2\ell^{\prime\prime}+1)M_{\ell^{\prime\prime}}\begin{pmatrix}\ell & \ell^\prime & \ell^{\prime\prime} \ 2 & -2 & 0 \end{pmatrix}^2 \frac{1\pm (-1)^{\ell+\ell^\prime + \ell^{\prime\prime}}}{2},
\end{equation}
where $M_{\ell^\prime\prime}$​ is the power spectrum of the mask.

It can be observed that the mask causes $\mu$ and $\varepsilon_\ell$​ to no longer depend solely on $S_\ell$ and $C_\ell^{BB, \rm ref}$​, but also to be influenced by ​$C_\ell^{\rm EE, \rm ref}$ and $M_\ell$. Accounting for these effects, the equivalent $\mu$ is no longer a scale-independent factor. The actual parameterization used in DroPS is:
\begin{equation}
  D_\ell(\nu, \nu^\prime) = W_d(\nu) W_d(\nu^\prime) \left\{ D_\ell^{\rm ref}\left[1+  \mu_\ell \left(\ln^2\frac{\nu}{\nu_{\rm ref}} + \ln^2\frac{\nu^\prime}{\nu_{\rm ref}} \right) \right] + \varepsilon_{\ell} \ln\frac{\nu}{\nu_{\rm ref}}\, \ln\frac{\nu^\prime}{\nu_{\rm ref}}\right\}, \label{eq:ME_ph}
\end{equation}
where $D_{\ell}^{\rm ref}$, $\mu_\ell$ and $\varepsilon_\ell$ (all in form of band powers) are directly taken as independent parameters. To avoid overfitting and consistently keep the $\mu_\ell$ and $\epsilon_\ell$ terms perturbative, we assume an upperbound for $|\mu_\ell|$ and $|\epsilon_\ell|$ (default: $0.05$).
...
Both the signal covariance and the signal$\times$noise covariance depend on the cosmology, particularly the value of $r$ (by default, DroPS assumes that variations in other cosmological parameters have a negligible impact on the B-mode power spectrum). Using the reference simulations at $r = 0$ and $r = r_1$, DroPS computes the signal covariance and the signal$\times$noise covariance for these two cases\footnote{Note that the foreground in the reference simulations is a fixed realization; therefore, the covariance matrices do not capture foreground sample variance. DroPS handles this foreground randomness by treating the foreground band powers at the reference frequency as free parameters and marginalizing over them, while additionally estimating the extra uncertainty introduced by frequency decorrelation using either the moment expansion or Taylor expansion method.}.

Since the signal (band power) exhibits an approximately linear dependence on $r$, within the range $0 < r < r_1$, DroPS interpolates the signal covariance linearly in $r^2$ and the signal$\times$noise covariance linearly in $r$. For $r > r_1$, DroPS simply uses the covariance matrix evaluated at $r = r_1$. The rationale for avoiding extrapolation of the covariance matrices is that extrapolating between two covariance matrices cannot guarantee positive definiteness, which can easily lead to catastrophic overfitting due to the emergence of negative eigenvalues.
...
\end{equation}
where $\mathbf{n}_j$ is the directional vector of the $j$-th pixel. The expansion bases $_{\pm 2}Y_{\ell m}$ are spin-weighted spherical harmonics. Note that here $c_{\ell m}^E$ and $c_{\ell m}^B$ are harmonic coefficients of the full-sky CMB map. Degeneracy between these parameters are expected for partial sky observations. Theoretically, selecting an orthonormal basis within the masked region would be computationally optimal. However, implementing this approach would lead to prohibitively complex code, as it requires solving for each individual mask and lacks support from readily available special function libraries. Consequently, DroPS abandons this theoretically feasible yet practically challenging solution.
