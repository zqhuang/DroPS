...
和
\begin{equation}
\mathcal{P}_T(k) = r A_s \left(\frac{k}{k{\mathrm{pivot}}}\right)^{n_t}.\label{eq:PT}
\end{equation}
...
在分析（无论是模拟的还是真实观测得到的）观测天图之前，必须模拟一批CMB、噪音和前景的参照数据组。参照数据组的模拟代表了目前我们对现实（仪器、噪音统计性质、前景统计性质等）的最佳认知。参照数据组可以类比为机器学习中的训练样本\footnote{注意这仅仅是一个类比，DroPS并不使用深度学习等前沿机器学习技术。}。 但无论如何，需要注意参照数据组可能和真实数据之间存在系统性的差别。如何尽可能地避免这些可能的差别带来的影响，是DroPS一个重要的优化指标。
...
\subsection{时序数据过滤和天图制作 \label{sec:TOD}}
...
\subsection{噪音模型 \label{sec:noise}}
