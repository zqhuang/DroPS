...
\item{For masked sky we are dealing with band powers with limited scale coverage, from which we cannot accurately evaluate the convolution in $\varepsilon_\ell$.}
\item{The mask and non-Gaussianity of the dust map may induce coupling between different modes. This will lead to extra scale-dependant contribution to $\mu$ and $\varepsilon_\ell$.}
\end{enumerate}
The net effect is then the relation between $\mu$, $C_\ell$  (in form of band powers)  and $S_\ell$  (also in form of band powers) becomes vague in the masked-sky case, and $\mu$ essentially becomes scale-dependent if mode mixing is not negligible. The final model of band powers used in DroPS is
\begin{equation}
...
\begin{figure}
  ...
  \caption{Comparison of base model (None), moment expansion method (ME) and Taylor expansion method(Taylor). Simulations are obtained with SO/SO\_sim\_config.txt. \label{fig:compare_methods}}
\end{figure}
