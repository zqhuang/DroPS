...
def plot_distr(filename, label, color,  cv):
    ...
    plt.hist(x[:, 0], bins=9, color=color[0], label="mean $r$ histogram, DroPS")
    plt.axvline(mean, color=color[1], ls = "solid", lw = 2.5, label=label+", DroPS",alpha=0.8)    
    plt.axvline(cv[0], color=color[2], ls = "dotted", lw = 2.5, label=label+", Wolz et al. Pipeline A",alpha=0.8)
    plt.axvline(cv[1], color=color[2], ls = "dashdot", lw = 2.5, label=label+", Wolz et al. Pipeline B",alpha=0.8)
    plt.axvline(cv[2], color=color[2], ls = "dashed", lw = 2.5, label=label+", Wolz et al. Pipeline C",alpha=0.8)    
...
mycolors = ['lightgray', '#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf']
