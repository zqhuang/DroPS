...
x_dev /= w
plt.imshow(x_dev, origin = 'lower', extent = (alpha_min, alpha_max, fd_min, fd_max) , cmap='seismic', vmax = 1.5, vmin=-1.5)
plt.colorbar(label=r"$r_{\rm bias}/\sigma_r$")
#plt.plot(x[ns-10:ns, 0], x[ns-10:ns, 1], color="black", alpha=0.3)
plt.scatter(x = x[:, 0], y = x[:, 1], s=0.4, c=(x[:, 3] - x[:,2])/x[:, 4], cmap='rainbow', vmin = -3., vmax = 3., alpha=0.5)
